...
const run = async () => {
  ...
  const browser = await puppeteer.launch({ headless: false }, { timeout: 50000 });
    ...
    if (isExistOnInnerSection(t) && isExistOnData(t)) {
      ...
      const scrappingItemData = await page.evaluate(async () => {
        ...
        const getAllOptions = (curr) => {
          ...
          console.log(
            ...
            curr?.children[0]?.children[1]?.children[0]?.children[0]?.children[0]?.children
          );
          ...
          return curr?.children[0]?.children[1]?.children[0]?.children[0]?.children[0]?.children?.map((innerCurr) => {
            ...
            // console.log("innerCurr 1 : ", innerCurr);
            // console.log(
            //   "innerCurr name : ",
            ...
            // console.log("innerCurr 3 : ", innerCurr?.children[0]?.children[0]);
            // console.log("innerCurr 4 : ", innerCurr?.children[0]?.children[0]?.children[0]);
            // console.log("innerCurr 5 : ", innerCurr?.children[0]?.children[0]?.children[0]?.children[0]);
      ...
      if (scrappingItemData?.length > 0) {
        scrapingMenuData = scrapingMenuData.map((mainItem) => {
          const i = { ...mainItem };
          if (mainItem.cache?.length > 0) {
            i.cache = [...mainItem.cache];
          } else {
            i.cache = [];
          }
  ...
  fs.writeFile("scrapingMenuData-v2.json", JSON.stringify(scrapingMenuData), (err) => {
    ...
    console.log("Data saved to scrapingMenuData.json");
  });
  fs.writeFile("allElementsData-v2.json", JSON.stringify(allElementsData), (err) => {
    if (err) {
      console.error(err);
      return;
    }
    console.log("Data saved to allElementsData-v2.json");
  });
